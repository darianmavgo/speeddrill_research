...
import (
	...
	ebitimage "github.com/ebitenui/ebitenui/image"
...
func (p *Piece) Draw(r *ebiten.Image, x, y int, angle Angle) {
	pieceinput := widget.NewTextInput(
		...
		widget.TextInputOpts.Image(&widget.TextInputImage{
			Idle:     ebitimage.NewNineSliceColor(color.NRGBA{R: 100, G: 100, B: 100, A: 255}),
			Disabled: ebitimage.NewNineSliceColor(color.NRGBA{R: 100, G: 100, B: 100, A: 255}),
		}),
	...
	for i := range p.blocks {
		for j := range p.blocks[i] {
			if p.isBlocked(i, j, angle) {
				...
				text.Draw(r, "E=mc^2", getArcadeFonts(1), x, y, color.Black)
